...
# Import the model
from models.LSTMVoltage_model import LSTMVoltageModel as VoltageModel  # Updated import
...
def objective(trial, config, device, train_loader, valid_loader, test_loader):
    ...
    project_name = f"LSTM_OptunaOptimization_{date_str}"
    ...
    save_path = os.path.join(PROJECT_ROOT, 'lstm_best_model.pth')
...
def run_training(config, device, train_loader, valid_loader, test_loader):
    date_str = get_current_date_str()
    project_name = f"LSTM_ManualSweep_{date_str}"
    ...
    # Add to config for wandb logging
